...
const SingleBlogPostPage: React.FC = () => {
  ...
  const { blogPosts, loadingContent, addCommentToItem, logContentActivity } = useContent(); 
  ...
  React.useEffect(() => {
    if (blogPostId && !loadingContent) {
        ...
        setPost(foundPost);
        if (foundPost) { setLikeCount(foundPost.likes || 0); }
    }
  ...
  return (
    <div className="pb-12">
      ...
      {isCommentModalOpen && post && (<CommentModal isOpen={isCommentModalOpen} onClose={() => setIsCommentModalOpen(false)} eventTitle={post.title} onSubmitComment={handleSubmitComment} isSubmitting={isSubmittingComment}/>)}
    </div>
  );
};
