...
import { BlogPost, BlogPostFormData, GenericContentFormData } from '../../types'; 
...
const ManageBlogPostsPage: React.FC = () => {
 ...
 const sortedPosts = React.useMemo(() =>
  ...
  const handleSubmit = async (data: GenericContentFormData) => {
    if (editingPost) {
      await updateContent('blogPost', editingPost.id, data as BlogPostFormData);
    } else {
      await addContent('blogPost', data as BlogPostFormData);
    }
    handleCloseModal();
  };
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this blog post?')) {
      await deleteContent('blogPost', id);
    }
  };
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold text-gray-800">Manage Blog Posts</h1>
        <Button onClick={() => handleOpenModal()} variant="primary" size="sm">
          <PlusIcon className="mr-1.5" /> Add Blog Post
        </Button>
      </div>

      {loadingContent && <p className="text-gray-500">Loading blog posts...</p>}
      {!loadingContent && blogPosts.length === 0 && (
         <Card><CardContent><p className="text-center text-gray-500 py-8">No blog posts found. Add one to get started!</p></CardContent></Card>
      )}
        ...
        {sortedPosts.map((post) => (
          <Card key={post.id} className="flex flex-col">
            ...
            <CardHeader className="flex-grow">
              <h2 className="text-lg font-semibold text-gray-700 truncate" title={post.title}>{post.title}</h2>
              <p className="text-xs text-gray-500">{formatDateADBS(post.date)}</p>
              <p className="text-xs text-purple-600 mt-1">{post.category}</p>
            </CardHeader>
             <CardContent className="py-2 px-4 h-20 overflow-hidden">
              <p className="text-xs text-gray-600 text-ellipsis">{post.description}</p>
            </CardContent>
            <CardFooter className="flex justify-between items-center p-3">
              <div className="flex space-x-3">
                <span className="text-xs text-gray-500 flex items-center" title={`${post.likes || 0} likes`}>
                    <HeartIconSolid className="w-4 h-4 mr-1 text-red-400" />
                    {post.likes || 0}
                </span>
                <span className="text-xs text-gray-500 flex items-center" title={`${post.comments?.length || 0} comments`}>
                    <ChatBubbleLeftEllipsisIcon className="w-4 h-4 mr-1 text-blue-400" />
                    {post.comments?.length || 0}
                </span>
              </div>
              <div className="space-x-2">
                <Button variant="outline" size="sm" onClick={() => handleOpenModal(post)}>Edit</Button>
                <Button variant="secondary" size="sm" onClick={() => handleDelete(post.id)} className="!bg-red-500 hover:!bg-red-600 text-white">Delete</Button>
              </div>
            </CardFooter>
          </Card>
        ))}
