...
import { Sermon, SermonFormData, GenericContentFormData } from '../../types'; 
    ...
    <div className="w-full bg-white text-slate-800 rounded-2xl p-4 sm:p-6"> 
      <div className="flex justify-between items-center mb-6">
        ...
        <Button onClick={() => handleOpenModal()} variant="primary" size="sm">
          <PlusIcon className="mr-1.5" /> Add Sermon
        </Button>
      </div>
        ...
        {sortedSermons.map((sermon) => (
          <Card key={sermon.id} className="flex flex-col">
            ...
            <CardHeader className="flex-grow">
              <h2 className="text-lg font-semibold text-gray-700 truncate" title={sermon.title}>{sermon.title}</h2>
              <p className="text-xs text-gray-500">{sermon.speaker || 'N/A'} - {formatDateADBS(sermon.date)}</p>
              <p className="text-xs text-purple-600 mt-1">{sermon.category}</p>
            </CardHeader>
            <CardContent className="py-2 px-4 h-20 overflow-hidden">
              <p className="text-xs text-gray-600 text-ellipsis">{sermon.description}</p>
            </CardContent>
            <CardFooter className="flex justify-between items-center p-3">
              <div className="flex space-x-3">
                <span className="text-xs text-gray-500 flex items-center" title={`${sermon.likes || 0} likes`}>
                    <HeartIconSolid className="w-4 h-4 mr-1 text-red-400" />
                    {sermon.likes || 0}
                </span>
                <span className="text-xs text-gray-500 flex items-center" title={`${sermon.comments?.length || 0} comments`}>
                    <ChatBubbleLeftEllipsisIcon className="w-4 h-4 mr-1 text-blue-400" />
                    {sermon.comments?.length || 0}
                </span>
              </div>
              <div className="space-x-2">
                <Button variant="outline" size="sm" onClick={() => handleOpenModal(sermon)}>Edit</Button>
                <Button variant="secondary" size="sm" onClick={() => handleDelete(sermon.id)} className="!bg-red-500 hover:!bg-red-600 text-white">Delete</Button>
              </div>
            </CardFooter>
          </Card>
        ))}
