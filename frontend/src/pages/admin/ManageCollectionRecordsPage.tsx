// frontend/src/pages/admin/ManageCollectionRecordsPage.tsx
import React, { useState, useMemo, useEffect } from 'react';
import { useContent } from '../../contexts/ContentContext';
import Card, { CardContent, CardHeader, CardFooter } from '../../components/ui/Card';
import Button from '../../components/ui/Button';
import ContentFormModal from '../../components/admin/ContentFormModal';
import { CollectionRecord, CollectionRecordFormData, GenericContentFormData, collectionPurposeList, CollectionPurpose } from '../../types';
import { formatDateADBS } from '../../dateConverter';
import * as XLSX from 'xlsx';
import {
  PlusIcon as HeroPlusIcon,
  DocumentTextIcon as DocumentPdfIcon,
  TableCellsIcon as DocumentCsvIcon,
  Squares2X2Icon,
  Bars3Icon,
  CheckCircleIcon,
...
const safeFilenamePiece = (v: unknown) =>
  String(v ?? '')
    .trim()
    .replace(/\s+/g, '_')
    .replace(/[^\w\-]+/g, '_');
...
export const ManageCollectionRecordsPage: React.FC = () => {
  ...
  const filteredRecords = useMemo(() => {
    return collectionRecords
      .filter((record: any) => {
        ...
        const searchMatch =
          String(record.collectorName ?? '').toLowerCase().includes(term) ||
          String(record.purpose ?? '').toLowerCase().includes(term) ||
          String(record.source ?? '').toLowerCase().includes(term) ||
          String(record.notes ?? '').toLowerCase().includes(term) ||
          String(record.amount ?? '').includes(term) ||
          (record.donors || []).some((donor: any) => String(donor.donorName ?? '').toLowerCase().includes(term));

        return purposeMatch && dateMatch && searchMatch;
      })
      .sort((a: any, b: any) => new Date(b.collectionDate).getTime() - new Date(a.collectionDate).getTime());
  ...
  const totalAmountForFiltered = useMemo(() => {
    return filteredRecords.reduce((sum: number, record: any) => sum + Number(record.amount ?? 0), 0);
  }, [filteredRecords]);
  ...
  const handleSubmit = async (data: GenericContentFormData) => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        const title =
          (result.newItem as CollectionRecord)?.purpose ||
          (result.updatedItem as CollectionRecord)?.purpose ||
          'Record';
        const rawAmount =
          (result.newItem as CollectionRecord)?.amount ||
          (result.updatedItem as CollectionRecord)?.amount ||
          0;
  ...
  const downloadRecordPdf = async (record: CollectionRecord) => {
    const purpose = safeFilenamePiece((record as any).purpose || 'Record');
    const shortId = String((record as any).id || '').slice(0, 6) || 'id';
    await downloadBackendPdf(`/api/pdfs/collection-records/${record.id}`, `Collection_Record_${purpose}_${shortId}.pdf`);
  };
    ...
    <Card key={record.id} className="flex flex-col dark:bg-slate-800">
      <CardHeader className="pb-3 dark:border-slate-700">
        <div className="flex justify-between items-start">
          <h2 className="text-md font-semibold text-gray-800 dark:text-slate-100 flex-grow mr-2" title={(record as any).purpose}>
            {(record as any).purpose}
          </h2>
          <span className="text-lg font-bold text-purple-600 dark:text-purple-400 whitespace-nowrap">
            NPR {Number((record as any).amount ?? 0).toFixed(2)}
          </span>
        </div>
        <p className="text-xs text-gray-500 dark:text-slate-400">Date: {formatDateADBS((record as any).collectionDate)}</p>
        <div className="flex items-center gap-2 mt-1">
          <span className="text-xs text-amber-700 dark:text-amber-500 bg-amber-100 dark:bg-amber-700/30 px-2 py-0.5 rounded-full inline-block">
            {(record as any).collectorName}
          </span>
          {(record as any).isDeposited ? (
            <span className="text-xs font-medium text-green-700 bg-green-100 px-2 py-0.5 rounded-full flex items-center">
      ...
      <CardContent className="text-xs text-gray-600 dark:text-slate-300 space-y-1 flex-grow pt-2 pb-3">
        {(record as any).source && (
          <p>
            <strong>Source:</strong> {(record as any).source}
          </p>
        )}

        {(record as any).donors && (record as any).donors.length > 0 && (
        ...
        {(record as any).notes && (
          <p className="mt-1 italic line-clamp-2">
            <strong>Notes:</strong> {(record as any).notes}
          </p>
        )}
      </CardContent>
  ...
  return (
    <div className="w-full">
      ...
      <div className="mb-4 text-right text-sm font-semibold text-gray-700 dark:text-slate-200">
        Total for Filtered:{' '}
        <span className="text-purple-700 dark:text-purple-400">NPR {totalAmountForFiltered.toFixed(2)}</span>
      </div>
      ...
      {viewMode === 'card' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {paginatedRecords.map((record: any) => renderRecordCard(record))}
        </div>
      ) : (
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
              ...
              <tbody className="bg-white dark:bg-slate-900 divide-y divide-gray-200 dark:divide-slate-700">
                {paginatedRecords.map((record: any) => (
                  <tr key={record.id}>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600 dark:text-slate-200">{formatDateADBS(record.collectionDate)}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600 dark:text-slate-200">{record.purpose}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-600 dark:text-slate-200">{record.collectorName}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-slate-300">{record.source || '—'}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700 dark:text-slate-100 text-right font-semibold">{Number(record.amount ?? 0).toFixed(2)}</td>
