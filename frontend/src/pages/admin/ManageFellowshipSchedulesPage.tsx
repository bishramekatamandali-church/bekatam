...
const ManageFellowshipSchedulesPage: React.FC = () => {
      ...
      [...fellowshipRosters]
        .filter(roster => {
          ...
          return (
            ...
            (roster.responsibilities || []).some(r => r.assignedTo.toLowerCase().includes(term) || r.role.toLowerCase().includes(term))
          );
        })
      ...
      [...generatedSchedules]
        .filter(schedule => {
          ...
          return (
            ...
            (schedule.responsibilities || []).some(r => r.assignedTo.toLowerCase().includes(term) || r.role.toLowerCase().includes(term))
          );
        })
  ...
  const handleOpenRosterModal = (roster?: FellowshipRosterItem) => {
    setEditingRoster(roster || null);
  ...
  const handleRosterDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this schedule entry?')) await deleteContent('fellowshipRoster', id);
  };

  // Excel helpers (Rosters)
  const rosterItemToExcelRow = (item: FellowshipRosterItem) => [
    item.id,
    item.rosterType,
    item.groupNameOrEventTitle,
    formatDateADBS(item.assignedDate),
    item.timeSlot,
    ...
    item.location || '',
    item.contactNumber || '',
    item.additionalNotesOrProgramDetails || '',
    item.isTemplate ? 'Yes' : 'No',
    item.postedByAdminName || '',
    formatTimestampADBS(item.createdAt || ''),
  ];
  ...
  const handleOpenEditDraftModal = (draft: GeneratedScheduleItem) => {
  ...
  const handleDraftUpdateSubmit = async (draftId: string, data: Partial<GeneratedScheduleItem>) => {
    if (await updateGeneratedSchedule(draftId, data)) handleCloseEditDraftModal();
  ...
  const handleDownloadSchedulePDF = async (schedule: GeneratedScheduleItem) => {
    setActiveDropdownId(null);
    await downloadSchedulePdf(schedule.id);
  };

  // Excel helpers (Schedules)
  const scheduleToExcelRow = (item: GeneratedScheduleItem) => [
    item.id,
    item.rosterType,
    item.groupNameOrEventTitle,
    formatDateADBS(item.scheduledDate),
    item.timeSlot,
    getResponsibility(item.responsibilities, 'coordinator'),
    getResponsibility(item.responsibilities, 'co-coordinator'),
    getResponsibility(item.responsibilities, 'choir'),
    ...
    item.additionalNotesOrProgramDetails || '',
    item.adminNotes || '',
    item.isPublishedAsEvent ? 'Yes' : 'No',
    item.publishedEventId || '',
    formatTimestampADBS(item.generatedAt),
  ];
  ...
  const renderSearchAndToggle = (
    ...
    <div className="mb-4 p-3 bg-slate-100 dark:bg-slate-800/50 rounded-lg shadow-sm flex flex-col sm:flex-row gap-3 items-center">
      ...
      <div className="flex items-center gap-1 self-start sm:self-center">
        <Button onClick={() => setViewMode('card')} variant={currentViewMode === 'card' ? 'primary' : 'ghost'} size="sm" className="!p-1.5" aria-label="Card View">
          <HeroSquares2X2Icon className="w-5 h-5" />
        </Button>
        <Button onClick={() => setViewMode('list')} variant={currentViewMode === 'list' ? 'primary' : 'ghost'} size="sm" className="!p-1.5" aria-label="List View">
          <HeroListBulletIcon className="w-5 h-5" />
        </Button>
      </div>
    ...
    return (
      <Card key={roster.id} className="flex flex-col dark:bg-slate-800">
        <CardHeader className="dark:border-slate-700 pb-2">
          <h3 className="font-semibold text-slate-700 dark:text-slate-200 truncate" title={roster.groupNameOrEventTitle}>
            {roster.groupNameOrEventTitle}
          </h3>
        ...
        <CardContent className="text-xs text-slate-600 dark:text-slate-300 space-y-1 flex-grow pt-1 pb-2">
          {coordinator && (
            <p className="truncate">
              <strong>Coordinator:</strong> {coordinator}
            </p>
          )}
          {speaker && (
            <p className="truncate">
              <strong>Speaker/Program:</strong> {speaker}
            </p>
          )}
          {roster.location && (
            <p className="truncate">
        ...
        <CardFooter className="flex justify-end space-x-2 bg-gray-50 dark:bg-slate-700/50 p-2">
          <Button asLink to={roster.linkPath || '#'} variant="outline" size="sm" className="text-xs dark:text-slate-300 dark:border-slate-600 dark:hover:bg-slate-700">
            <HeroEyeIcon className="w-4 h-4" />
          </Button>

          {/* Roster-item PDF removed (backend migration + removed roster-item PDFs) */}

          <Button variant="outline" size="sm" onClick={() => handleOpenRosterModal(roster)} className="text-xs dark:text-slate-300 dark:border-slate-600 dark:hover:bg-slate-700">
            Edit
          </Button>
          <Button variant="secondary" size="sm" onClick={() => handleRosterDelete(roster.id)} className="!bg-red-500 hover:!bg-red-600 text-white text-xs">
            Delete
          </Button>
        </CardFooter>
      </Card>
      ...
      <Card key={schedule.id} className="flex flex-col dark:bg-slate-800">
        <CardHeader className="dark:border-slate-700 pb-2">
          <div className="flex justify-between items-start">
            <h3 className="font-semibold text-slate-700 dark:text-slate-200 truncate flex-grow mr-2" title={schedule.groupNameOrEventTitle}>
              {schedule.groupNameOrEventTitle}
            </h3>
            <span className={`text-xs px-2 py-0.5 rounded-full ml-2 ${schedule.isPublishedAsEvent ? 'bg-green-100 text-green-700 dark:bg-green-700/50 dark:text-green-300' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-700/50 dark:text-yellow-300'}`}>
              {schedule.isPublishedAsEvent ? 'Published' : 'Draft'}
            </span>
          </div>
        ...
        <CardFooter className="flex justify-end space-x-2 bg-gray-50 dark:bg-slate-700/50 p-2">
          <Button asLink to={schedule.linkPath || '#'} variant="outline" size="sm" className="text-xs dark:text-slate-300 dark:border-slate-600 dark:hover:bg-slate-700">
            <HeroEyeIcon className="w-4 h-4" />
          </Button>
          <Button variant="outline" size="sm" onClick={() => handleOpenEditDraftModal(schedule)} className="text-xs dark:text-slate-300 dark:border-slate-600 dark:hover:bg-slate-700">
            Edit
          </Button>
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-wrap border-b border-slate-200 dark:border-slate-700 mb-6">
        <button
          onClick={() => setActiveTab('inputtedRosters')}
        ...
        <button
          onClick={() => setActiveTab('generatedSchedules')}
          className={`px-4 py-2 text-sm font-medium border-b-2 ${
      ...
      {activeTab === 'inputtedRosters' && (
        <section>
          <div className="flex justify-between items-center mb-4">
            ...
            <Button onClick={() => handleOpenRosterModal()} variant="primary" size="sm">
              <HeroPlusIcon className="w-4 h-4 mr-1.5" /> Add New Schedule
            </Button>
          </div>
          ...
          {loadingContent ? (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y dark:divide-slate-700">
                <thead>{/* List view table headers here */}</thead>
                <tbody>{/* List view table rows here */}</tbody>
              </table>
            </div>
          )}
        </section>
      ...
      {activeTab === 'generatedSchedules' && (
        <section>
          ...
          {loadingContent ? (
            <p>Loading...</p>
          ) : filteredGeneratedSchedules.length === 0 ? (
            <p>No draft schedules available yet.</p>
      ...
      {isEditDraftModalOpen && editingDraft && (
        <EditScheduleDraftModal isOpen={isEditDraftModalOpen} onClose={handleCloseEditDraftModal} onSubmit={handleDraftUpdateSubmit} initialData={editingDraft} isLoading={loadingContent} />
      )}
    </div>
  );
};
