...
import { NewspaperIcon as HeroNewspaperIcon, SpeakerWaveIcon as HeroSpeakerWaveIcon, CalendarIcon as HeroCalendarIcon } from '@heroicons/react/24/outline'; // Adjusted to use Outline for consistency where solid wasn't used
import { PrayerHandsIcon, TestimonyIcon } from '../../components/icons/GenericIcons';
...
const ReceiptPercentIcon: React.FC<{ className?: string }> = ({ className }) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={className}>
    ...
    <path d="M3.75 3A1.75 1.75 0 002 4.75v14.5A1.75 1.75 0 003.75 21h12.5A1.75 1.75 0 0018 19.25V4.75A1.75 1.75 0 0016.25 3H3.75zm.75 1.5h11a.25.25 0 01.25.25v14.5a.25.25 0 01-.25.25h-11a.25.25 0 01-.25-.25V4.75a.25.25 0 01.25-.25z" />
    <path d="M8.25 8.625a1.125 1.125 0 100-2.25 1.125 1.125 0 000 2.25zM11.75 14a1.125 1.125 0 10-2.25 0 1.125 1.125 0 002.25 0z" />
  </svg>
);
...
const AdminDashboardPage: React.FC = () => {
  ...
  const groupOrder = ["General", "Content Management", "Media Management", "Community Engagement", "About Page Settings", "Church Records", "Program Management", "Membership & Admin", "Finance", "Site Administration", "Other"];
  ...
  const navLinkClasses = ({ isActive }: { isActive: boolean }) =>
    `flex items-center px-3 py-2 text-sm font-semibold rounded-lg border transition-all duration-150 ${isActive
      ? 'bg-white text-slate-900 border-slate-300 shadow-[0_6px_14px_rgba(0,0,0,0.12)] ring-1 ring-purple-200'
      : 'bg-white text-slate-700 border-slate-200 shadow-[0_4px_10px_rgba(0,0,0,0.08)] hover:bg-slate-50'
    }`;
  ...
  return (
    <div className="relative min-h-screen w-full bg-white">
      ...
      <div className="relative z-30 px-3 py-3 sm:px-6 sm:py-6">
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm min-h-[calc(100vh-3rem)]">
          <div className="flex flex-wrap items-center justify-between gap-3 px-4 sm:px-6 py-3 border-b border-slate-200 sticky top-0 bg-white rounded-t-2xl">
            <div>
              <p className="text-[11px] uppercase tracking-[0.25em] text-slate-400">Workspace</p>
              <h2 className="text-xl font-semibold text-slate-800">{isOverviewRoute ? 'Dashboard Overview' : activeTab?.label || 'Admin Function'}</h2>
            </div>
      ...
      <aside className={`fixed top-0 right-0 bottom-0 w-full sm:w-96 bg-white shadow-2xl z-50 transform transition-transform duration-200 ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-4 border-b border-slate-200 flex items-center justify-between">
          <div>
            <p className="text-[11px] uppercase tracking-[0.25em] text-slate-400">Admin menu</p>
            <h3 className="text-lg font-semibold text-slate-800">Choose a function</h3>
          </div>
          <button
            onClick={() => setIsMenuOpen(false)}
        ...
        <div className="p-4 space-y-4 overflow-y-auto h-full pb-24">
          {sortedGroupEntries.map(([groupName, tabsInGroup]) => (
            <div key={groupName} className="space-y-2">
              {groupName !== 'Other' && (
                <h3 className="text-xs font-semibold text-white uppercase tracking-wide bg-gradient-to-r from-purple-600 to-blue-500 px-3 py-1 rounded-md">{groupName}</h3>
              )}
