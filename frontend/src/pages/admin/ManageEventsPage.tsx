...
export const ManageEventsPage: React.FC = () => {
  ...
  const handleSubmit = async (data: GenericContentFormData) => {
    if (editingEvent) {
      await updateContent('event', editingEvent.id, data as EventFormData);
    } else {
      await addContent('event', data as EventFormData);
    }
    handleCloseModal();
  };
  ...
  return (
    <div className="w-full">
      ...
      {!loadingContent && events.length === 0 && (
        <Card>
          <CardContent>
            <p className="text-center text-gray-500 py-8">No events found. Add one to get started!</p>
          </CardContent>
        </Card>
      )}
        ...
        {sortedEvents.map((event) => (
          <Card key={event.id} className="flex flex-col">
            ...
            <CardHeader className="flex-grow">
              <h2 className="text-lg font-semibold text-gray-700 truncate" title={event.title}>{event.title}</h2>
              <p className="text-xs text-gray-500">{formatDateADBS(event.date)} at {event.time}</p>
              <p className="text-xs text-purple-600 mt-1">{event.category}</p>
            </CardHeader>
            <CardContent className="py-2 px-4 h-20 overflow-hidden">
              <p className="text-xs text-gray-600 text-ellipsis">{event.description}</p>
            </CardContent>
            <CardFooter className="flex justify-between items-center p-3">
              <div className="flex space-x-3">
                <span className="text-xs text-gray-500 flex items-center" title={`${event.likes || 0} likes`}>
                  <HeartIconSolid className="w-4 h-4 mr-1 text-red-400" />
                  {event.likes || 0}
                </span>
                <span className="text-xs text-gray-500 flex items-center" title={`${event.comments?.length || 0} comments`}>
