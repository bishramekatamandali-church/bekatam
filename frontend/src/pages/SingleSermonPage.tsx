...
const SingleSermonPage: React.FC = () => {
  ...
  const { sermons, loadingContent, addCommentToItem, logContentActivity } = useContent();
  ...
  return (
    <div className="pb-12">
      <div className="container mx-auto px-4">
        <Card className="max-w-4xl mx-auto">
          ...
          {!youtubeEmbedUrl && sermon.videoUrl && (
            <div className="bg-black rounded-t-xl overflow-hidden"><video src={sermon.videoUrl} controls className="w-full max-h-[500px] object-contain" aria-label={`Video player for ${sermon.title}`}/></div>
          )}
          {sermon.imageUrl && (<div className={`${!hasVideo ? 'bg-black rounded-t-xl overflow-hidden' : 'mt-4'}`}><img src={sermon.imageUrl} alt={sermon.title} className={`w-full h-auto object-cover ${!hasVideo ? 'max-h-[500px]' : 'max-h-[400px] rounded-lg'}`}/></div>)}
          
          <CardHeader className={`${!(hasVideo || sermon.imageUrl) ? 'rounded-t-xl' : ''}`}>
          ...
          <CardContent>
            ...
            <div className="mt-8 pt-6 border-t border-gray-200 flex justify-around items-center">
                ...
                <Button variant="ghost" onClick={handleAddCommentClick} className="flex items-center text-slate-600 hover:text-teal-500"><ChatBubbleOvalLeftEllipsisIcon className="w-5 h-5 mr-1.5" /> {currentCommentCount} <span className="ml-1 hidden sm:inline">Comment</span></Button>
                <Button variant="ghost" onClick={() => setIsShareModalOpen(true)} className="flex items-center text-slate-600 hover:text-teal-500"><ShareIconUI className="w-5 h-5 mr-1.5" /> <span className="hidden sm:inline">Share</span></Button>
            </div>
      ...
      {isCommentModalOpen && sermon && (<CommentModal isOpen={isCommentModalOpen} onClose={() => setIsCommentModalOpen(false)} eventTitle={sermon.title} onSubmitComment={handleSubmitComment} isSubmitting={isSubmittingComment}/>)}
    </div>
  );
};
