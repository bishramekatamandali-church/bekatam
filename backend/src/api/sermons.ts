...
const getMissingSermonColumns = (error: unknown) => {
  ...
  if (isMissingColumnError(error, 'postedByAdminId')) missing.add('postedByAdminId');
  if (isMissingColumnError(error, 'postedByAdminName')) missing.add('postedByAdminName');
  if (isMissingColumnError(error, 'location')) missing.add('location');
  return missing;
};
  ...
  try {
    ...
    if (missingColumns.size > 0) {
      try {
        ...
        if (handleDatabaseFallback(req, res, fallbackError)) return;
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    await prisma.contentlike.deleteMany({ where: { itemType: 'sermon', itemId: id } });
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === 'P2025') return res.status(404).json({ error: 'Sermon to delete not found.' });
