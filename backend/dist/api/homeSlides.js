...
const shapeHomeSlideForFrontend = (slide) => ({
    ...slide,
...
router.get('/', async (req, res) => {
    try {
        const slides = await db_1.prisma.homeslide.findMany({
            orderBy: { order: 'asc' },
        });
...
router.post('/', async (req, res) => {
    ...
    try {
        const newSlide = await db_1.prisma.homeslide.create({
            data: {
...
router.put('/:id', async (req, res) => {
    ...
    const { title, description, imageUrl, ctaText, linkPath, order, isActive } = req.body;
    try {
        const updatedSlide = await db_1.prisma.homeslide.update({
            where: { id },
            data: {
                title,
                description,
...
router.delete('/:id', async (req, res) => {
    const { id } = req.params;
    try {
        await db_1.prisma.homeslide.delete({ where: { id } });
        res.status(204).send();
    }
    catch (error) {
        if (error instanceof client_1.Prisma.PrismaClientKnownRequestError && error.code === 'P2025') {
