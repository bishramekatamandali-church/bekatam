...
class _AdminAdvertisementsScreenState extends ConsumerState<AdminAdvertisementsScreen> {
  ...
  Future<void> _openForm({Map<String, dynamic>? existing}) async {
    ...
    await showModalBottomSheet(
      ...
      builder: (ctx) => _AdFormSheet(existing: existing, adminId: profile?.id, adminName: profile?.fullName),
    );
    _load();
  }
...
class _AdFormSheetState extends State<_AdFormSheet> {
  late final TextEditingController _name, _imageUrl, _videoUrl, _linkUrl, _altText, _displayOrder, _adSizeKey;
  ...
  Future<void> _generateWithAI() async {
    if (_linkUrl.text.trim().isEmpty) {
      setState(() => _error = 'Enter a link URL first so the AI can analyze the page.');
      return;
    }
    setState(() {
    ...
    try {
      ...
      setState(() {
        _error = 'AI generation failed: $e (make sure GEMINI_API_KEY is configured on the project)';
        _generatingCopy = false;
      });
    }
  }
  ...
  Future<void> _save() async {
    ...
    final body = <String, dynamic>{
      ...
      'ad_size_key': _adSizeKey.text.trim().isEmpty ? null : _adSizeKey.text.trim(),
      'is_active': _isActive,
    };
    try {
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: _adTypes.map((t) => DropdownMenuItem(value: t, child: Text(t.replaceAll('_', ' ')))).toList(),
              onChanged: (v) => setState(() => _adType = v ?? 'image_banner'),
            ),
            ...
            const SizedBox(height: 8),
            TextField(controller: _displayOrder, decoration: const InputDecoration(labelText: 'Display Order'), keyboardType: TextInputType.number),
            const SizedBox(height: 8),
