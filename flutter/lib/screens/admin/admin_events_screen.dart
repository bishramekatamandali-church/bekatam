...
/// Ports the React event editor while preserving the JSONB fields from the
/// Supabase schema. The advanced JSON fields intentionally edit arrays as JSON
/// so existing multi-location, conducted-by, and speaker data is not lost.
              ...
              : RefreshIndicator(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, i) {
                      final item = _items[i];
                      final dateStr = item['date'] != null ? DateFormat.yMMMd().format(DateTime.tryParse(item['date']) ?? DateTime.now()) : '';
                      return Card(
...
class _EventFormSheetState extends State<_EventFormSheet> {
  late final TextEditingController _title, _description, _imageUrl, _linkPath, _location, _time,
      ...
      _scheduleNotes, _locationsJson, _conductedByJson, _speakersJson, _videoUrl, _audioUrl;
  String? _category;
  ...
  void initState() {
    ...
    _scheduleNotes = TextEditingController(text: e?['schedule_notes'] ?? '');
    _locationsJson = TextEditingController(text: _jsonText(e?['locations']));
    _conductedByJson = TextEditingController(text: _jsonText(e?['conducted_by']));
    _speakersJson = TextEditingController(text: _jsonText(e?['speakers']));
  ...
  @override
  void dispose() {
    for (final c in [_title, _description, _imageUrl, _linkPath, _location, _time, _expectations, _guests, _contactPerson, _contactEmail, _contactPhone, _registrationLink, _capacity, _feeAmount, _scheduleNotes, _locationsJson, _conductedByJson, _speakersJson, _videoUrl, _audioUrl]) { c.dispose(); }
    super.dispose();
  }

  dynamic _parseJsonArray(String value, String label) {
    final decoded = jsonDecode(value);
    if (decoded is! List) throw FormatException('$label must be a JSON array.');
    return decoded;
  }
  ...
  Future<void> _save() async {
    ...
    try {
      final body = <String, dynamic>{
        ...
        'category': _category, 'event_type': _eventType, 'schedule_type': _scheduleType,
        'schedule_notes': _scheduleNotes.text.trim().isEmpty ? null : _scheduleNotes.text.trim(),
        'date': _date?.toIso8601String(), 'location': _location.text.trim().isEmpty ? null : _location.text.trim(),
        'locations': _parseJsonArray(_locationsJson.text, 'Locations'),
        'conducted_by': _parseJsonArray(_conductedByJson.text, 'Conducted by'),
        'speakers': _parseJsonArray(_speakersJson.text, 'Speakers'),
        ...
        'capacity': int.tryParse(_capacity.text.trim()), 'is_fee_required': _isFeeRequired,
      ...
      if (widget.existing == null) {
        body['posted_by_admin_id'] = widget.adminId; body['posted_by_admin_name'] = widget.adminName; body['likes'] = 0;
  ...
  Widget _jsonField(TextEditingController c, String label) => Padding(
    padding: const EdgeInsets.only(top: 8),
    child: TextField(controller: c, decoration: InputDecoration(labelText: label, helperText: 'JSON array; existing values are preserved'), maxLines: 4),
  );
    ...
    return Padding(
      ...
      child: SingleChildScrollView(
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          const SizedBox(height: 8),
          DropdownButtonFormField<String>(value: _category, decoration: const InputDecoration(labelText: 'Category'), items: _eventCategories.map((c) => DropdownMenuItem(value: c, child: Text(c.replaceAll('_', ' ')))).toList(), onChanged: (v) => setState(() => _category = v)),
          const SizedBox(height: 8),
          DropdownButtonFormField<String>(value: _eventType, decoration: const InputDecoration(labelText: 'Event Type'), items: _eventTypes.map((c) => DropdownMenuItem(value: c, child: Text(c))).toList(), onChanged: (v) => setState(() => _eventType = v ?? 'REGULAR')),
          const SizedBox(height: 8),
          DropdownButtonFormField<String>(value: _scheduleType, decoration: const InputDecoration(labelText: 'Schedule Type'), items: _scheduleTypes.map((c) => DropdownMenuItem(value: c, child: Text(c.replaceAll('_', ' ')))).toList(), onChanged: (v) => setState(() => _scheduleType = v)),
          TextField(controller: _scheduleNotes, decoration: const InputDecoration(labelText: 'Schedule Notes'), maxLines: 2),
          const SizedBox(height: 8),
          ListTile(contentPadding: EdgeInsets.zero, title: Text(_date == null ? 'Pick a date' : DateFormat.yMMMd().format(_date!)), trailing: const Icon(Icons.edit_calendar), onTap: () async { final picked = await showDatePicker(context: context, firstDate: DateTime(2000), lastDate: DateTime(2100), initialDate: _date ?? DateTime.now()); if (picked != null) setState(() => _date = picked); }),
          TextField(controller: _time, decoration: const InputDecoration(labelText: 'Time (e.g. 10:00 AM)')),
          const SizedBox(height: 8),
          TextField(controller: _location, decoration: const InputDecoration(labelText: 'Primary Location')),
          _jsonField(_locationsJson, 'Locations JSON'),
          _jsonField(_conductedByJson, 'Conducted By JSON'),
          _jsonField(_speakersJson, 'Speakers JSON'),
          const SizedBox(height: 8),
          ...
          SwitchListTile(contentPadding: EdgeInsets.zero, title: const Text('Fee required?'), value: _isFeeRequired, onChanged: (v) => setState(() => _isFeeRequired = v)),
          ...
          FilledButton(onPressed: _saving ? null : _save, child: _saving ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2)) : const Text('Save')),
        ]),
      ),
    );
  }
