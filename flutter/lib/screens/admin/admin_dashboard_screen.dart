...
/// Admin hub for the Flutter/Supabase replacement application.
...
class _OverviewCard extends StatefulWidget {
  const _OverviewCard();
  @override State<_OverviewCard> createState() => _OverviewCardState();
}
class _OverviewCardState extends State<_OverviewCard> {
  ...
  Future<void> _broadcast() async {
    ...
    try {
      ...
      final rows = (users as List).map((u) => {'target_user_id': u['id'], 'message': '✨ New Feature Update: $message', 'link': '/', 'type': 'feature_update', 'read': false}).toList();
      if (rows.isNotEmpty) await SupabaseService.client.from('notification').insert(rows);
      await AdminLogService.log(action: 'Broadcasted Feature Update', details: 'Message: $message');
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Feature update sent to ${rows.length} users.')));
    } catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Broadcast failed: $e'))); }
